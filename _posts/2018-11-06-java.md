...
### supper关键字
 ...
 - 子类通过构造器实例化时需要调用父类的默认构造器，是隐含的调用了supper()这个方法，但是如果子类的构造器
   调用了父类的有参数的构造器，那么无参数的构造器就不会被调用了，
 - 子类调用父类的默认构造器（父类如果存在有参数的构造器，一定要把父类隐藏的构造器显式的定义出来）   
 - `supper()`的调用必须放在方法的`第一行`
 - supper可以表示父类的引用，我们可以谁用supper 和this来区分父类和子类中同名的属性。在子类中访问属性
   的查找顺序首先在子类中查找指定的属性名字，如果子类中没有该属性，那么就去父类中去查找该属性，方法也是一样
   的当子类和父类有同样的方法时通过子类调用这个相同的方法时调用的是子类中的方法。但是我们可以通过supper对父类的引用
   来调用父类的方法。
 -   
 this关键字
   - 代表当前类的指定实例的引用
...
class Child extends Parent{
    String name="小张";
    public Child(){
        //在子类的构造器中隐藏的调用了supper；
        //supper();
        //也可以调用父类的一个有参数的构造器，supper一定是放在第一行
        supper(name);
    ...
    public void printName(){
        ...
        System.out.printIn('调用父类的属性'+ supper.name);
    }
    this.eat();
    supper.eat();
